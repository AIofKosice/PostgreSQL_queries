...
DROP FUNCTION IF EXISTS get_manager_hierarchy(VARCHAR);
CREATE OR REPLACE FUNCTION get_manager_hierarchy(manager_name VARCHAR(20))
RETURNS TABLE (
        id           int,
        name         varchar(100),
        manager_id   int,
...
BEGIN
    ...
    WITH RECURSIVE cte AS (
        SELECT *
        FROM emp_details
        WHERE emp_details.name = manager_name
        UNION 
        SELECT e.*
        FROM emp_details e
...
SELECT * FROM get_manager_hierarchy('Asha');
