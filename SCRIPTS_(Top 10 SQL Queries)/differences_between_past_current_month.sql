...
with avg_sales as (
select year_id, month_id, to_char(order_date, 'MON') as mon, avg(sales) as avg_sales_per_month from sales_order s
where year_id in (2003, 2004)
group by year_id, month_id, to_char(order_date, 'MON')
order by year_id, month_id
)
select year_id, mon, avg_sales_per_month - LAG(avg_sales_per_month, 12, 0) OVER() from avg_sales
...
select
cte2.mon, cte2.avg_sales_per_month - cte1.avg_sales_per_month as diff
from cte cte1 
join cte cte2 on cte2.mon = cte1.mon
where cte2.year_id = 2004 and cte1.year_id = 2003
order by cte1.month_id
...
SELECT
    ...
    AVG(CASE WHEN year_id = 2004 THEN sales END) - AVG(CASE WHEN year_id = 2003 THEN sales END) as diff
FROM sales_order
GROUP BY to_char(order_date, 'MON'), month_id
ORDER BY month_id